...
class CTransaction:
    # Object Constructor
    def __init__(self, title, ISBN, TUP_ID, dateBorrowed, dateToReturn, status, refNum, borrower, author, librarian, \
                 fine):
        self.title = title
        self.ISBN = ISBN
        self.TUP_ID = TUP_ID
...
"""
    ...
    def submit():
        ...
        if indexBook < 0:
            ...
            if response:
                transaction = CTransaction(title, ISBN, TUP_ID, dateBorrowed, dateToReturn, status, refNum, borrower, \
                                           author, librarian, fine)
                addTransaction(transaction)
                saveTransaction()
                messagebox.showinfo("BORROW BOOK", "TRANSACTION SUCCESSFULLY SUBMITTED. PROCEED TO THE LIBRARIAN TO APPROVE TRANSACTION")
...
def recommendBooks(borrower):
    ...
    for book in bookList:
        if book.category in borrowed_categories and book.title not in recommendations and book.title not in \
                borrower.borrowedBook:
...
def displayTransaction():
    for transaction in transactionList:
        print(
            ...
            + transaction.status + " " + transaction.refNum + " " + transaction.borrower + " " \
            + transaction.author + " " + transaction.librarian + " " + transaction.fine)
...
def searchTransaction():
    ...
    for transaction in transactionList:
        ...
        if keyword.lower() in attributeValue.lower():
            print(transaction.title + " " + transaction.ISBN + " " + transaction.TUP_ID + " " + \
                  transaction.dateBorrowed + " " + transaction.dateToReturn + " " \
                  + transaction.status + " " + transaction.refNum + " " + transaction.borrower + " " + \
                  transaction.author + " " + transaction.librarian + " " + transaction.fine)
            foundMatch = True
...
def saveTransaction():
    ...
    with open(filename, "w", newline="") as csvfile:
        ...
        writer.writerow(["Title", "Author", "ISBN", "Edition", "Year Published", "Material", "Category", "Shelf No.",
                         "Total Stocks", "No. of Borrower"])

        # Write each transaction's data row
        for transaction in transactionList:
            writer.writerow([transaction.title, transaction.ISBN, transaction.TUP_ID, transaction.dateBorrowed,
                             transaction.dateToReturn, transaction.status, transaction.refNum, transaction.borrower,
                             transaction.author, transaction.librarian, transaction.fine])
...
def checkTransactionFields(title, ISBN, TUP_ID, dateBorrowed, dateToReturn, status, refNum, borrower, author, librarian,
                           fine):
    if (title == "" or
            ISBN == "" or
            TUP_ID == "" or
